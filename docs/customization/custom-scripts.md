...
Custom scripting was introduced to provide a way for users to execute custom logic from within the NetPoint UI. Custom scripts enable the user to directly and conveniently manipulate NetPoint data in a prescribed fashion. They can be used to accomplish myriad tasks, such as:

* Automatically populate new devices and cables in preparation for a new site deployment
* Create a range of new reserved prefixes or IP addresses
* Fetch data from an external source and import it to NetPoint

Custom scripts are Python code and exist outside of the official NetPoint code base, so they can be updated and changed without interfering with the core NetPoint installation. And because they're completely custom, there is no inherent limitation on what a script can accomplish.
...
Scripts comprise two core components: a set of variables and a `run()` method. Variables allow your script to accept user input via the NetPoint UI, but they are optional: If your script does not require any user input, there is no need to define any variables.

The `run()` method is where your script's execution logic lives. (Note that your script can have as many methods as needed: this is merely the point of invocation for NetPoint.)
...
A particular object within NetPoint. Each ObjectVar must specify a particular model, and allows the user to select one of the available instances. ObjectVar accepts several arguments, listed below.
...
http://netpoint/api/extras/scripts/example.MyReport/ \
