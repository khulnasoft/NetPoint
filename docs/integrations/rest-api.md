...
```json
{
  ...
  "assigned_object": {
    "id": 114771,
    "url": "http://netpoint/api/dcim/interfaces/114771/",
    "device": {
      "id": 2230,
      "url": "http://netpoint/api/dcim/devices/2230/",
      "name": "router1",
...
```json
{
    ...
    "site": {
        "id": 7,
        "url": "http://netpoint/api/dcim/sites/7/",
        "name": "Corporate HQ",
        "slug": "corporate-hq"
    },
    "group": {
        "id": 4,
        "url": "http://netpoint/api/ipam/vlan-groups/4/",
        "name": "Production",
        "slug": "production"
    },
    "vid": 101,
    ...
    "role": {
        "id": 9,
        "url": "http://netpoint/api/ipam/roles/9/",
        "name": "User Access",
        "slug": "user-access"
    },
    "description": "",
...
For example, when creating a new device, its rack can be specified by NetPoint ID (PK):
...
Together, these values identify a unique object in NetPoint. The assigned object (if any) is represented by the `assigned_object` attribute on the IP address model.
...
http://netpoint/api/ipam/ip-addresses/ \
--data '{
...
{
    "id": 56296,
    "url": "http://netpoint/api/ipam/ip-addresses/56296/",
    "assigned_object_type": "dcim.interface",
    "assigned_object_id": 69000,
    "assigned_object": {
        "id": 69000,
        "url": "http://netpoint/api/dcim/interfaces/69023/",
        "device": {
            "id": 2174,
            "url": "http://netpoint/api/dcim/devices/2174/",
            "name": "device105",
...
{
    "id": 13980,
    "url": "http://netpoint/api/ipam/prefixes/13980/",
    "family": {
        "value": 4,
        "label": "IPv4"
    },
    ...
    "site": {
        "id": 3,
        "url": "http://netpoint/api/dcim/sites/17/",
        "name": "Site 23A",
        "slug": "site-23a"
    },
    "vrf": null,
    ...
    "role": {
        "id": 17,
        "url": "http://netpoint/api/ipam/roles/17/",
        "name": "Staging",
        "slug": "staging"
    },
    "is_pool": false,
...
{
    "count": 2861,
    "next": "http://netpoint/api/dcim/devices/?limit=50&offset=50",
    "previous": null,
    "results": [
        {
            "id": 231,
...
```
http://netpoint/api/dcim/devices/?limit=100
```
...
{
    "count": 2861,
    "next": "http://netpoint/api/dcim/devices/?limit=100&offset=100",
    "previous": null,
    "results": [...]
}
```
...
To query NetPoint for a list of objects, make a `GET` request to the model's _list_ endpoint. Objects are listed under the response object's `results` parameter.

```no-highlight
curl -s -X GET http://netpoint/api/ipam/ip-addresses/ | jq '.'
```

```json
{
  "count": 42031,
  "next": "http://netpoint/api/ipam/ip-addresses/?limit=50&offset=50",
  "previous": null,
  "results": [
    {
      "id": 5618,
...
To query NetPoint for a single object, make a `GET` request to the model's _detail_ endpoint specifying its unique numeric ID.

!!! note
    Note that the trailing slash is required. Omitting this will return a 302 redirect.

```no-highlight
curl -s -X GET http://netpoint/api/ipam/ip-addresses/5618/ | jq '.'
```
...
{
  "id": 18691,
  "url": "http://netpoint/api/ipam/prefixes/18691/",
  "family": {
    "value": 4,
    "label": "IPv4"
  },
  ...
  "site": {
    "id": 6,
    "url": "http://netpoint/api/dcim/sites/6/",
    "name": "US-East 4",
    "slug": "us-east-4"
  },
  "vrf": null,
...
[
    {
        "id": 21,
        "url": "http://netpoint/api/dcim/sites/21/",
        "name": "Site 1",
        ...
    },
    {
        "id": 22,
        "url": "http://netpoint/api/dcim/sites/22/",
        "name": "Site 2",
        ...
    },
    {
        "id": 23,
        "url": "http://netpoint/api/dcim/sites/23/",
        "name": "Site 3",
        ...
    }
]
...
-H "Authorization: Token $TOKEN" \
-H "Content-Type: application/json" \
http://netpoint/api/ipam/prefixes/18691/ \
...
!!! note
    The bulk update of objects is an all-or-none operation, meaning that if NetPoint fails to successfully update any of the specified objects (e.g. due a validation error), the entire operation will be aborted and none of the objects will be updated.

### Deleting an Object

To delete an object from NetPoint, make a `DELETE` request to the model's _detail_ endpoint specifying its unique numeric ID. The `Authorization` header must be included to specify an authorization token, however this type of request does not support passing any data in the body.
...
NetPoint supports the simultaneous deletion of multiple objects of the same type by issuing a `DELETE` request to the model's list endpoint with a list of dictionaries specifying the numeric ID of each object to be deleted. For example, to delete sites with IDs 10, 11, and 12, issue the following request:
...
!!! note
    The bulk deletion of objects is an all-or-none operation, meaning that if NetPoint fails to delete any of the specified objects (e.g. due a dependency by a related object), the entire operation will be aborted and none of the objects will be deleted.

## Authentication

The NetPoint REST API primarily employs token-based authentication. For convenience, cookie-based authentication can also be used when navigating the browsable API.

### Tokens

A token is a unique identifier mapped to a NetPoint user account. Each user may have one or more tokens which he or she can use for authentication when making REST API requests. To create a token, navigate to the API tokens page under your user profile.
...
Each token contains a 160-bit key represented as 40 hexadecimal characters. When creating a token, you'll typically leave the key field blank so that a random key will be automatically generated. However, NetPoint allows you to specify a key in case you need to restore a previously deleted token to operation.

Additionally, a token can be set to expire at a specific time. This can be useful if an external client needs to be granted temporary access to NetPoint.
...
```
$ curl https://netpoint/api/dcim/sites/
{
...
{
    "id": 6,
    "url": "https://netpoint/api/users/tokens/6/",
    "display": "3c9cb9 (hankhill)",
    "user": {
        "id": 2,
        "url": "https://netpoint/api/users/users/2/",
        "display": "hankhill",
        "username": "hankhill"
    },
...
This header specifies the API version in use. This will always match the version of NetPoint installed. For example, NetPoint v3.4.2 will report an API version of `3.4`.
...
!!! note
    This header is included with _all_ NetPoint responses, although it is most practical when working with an API.
