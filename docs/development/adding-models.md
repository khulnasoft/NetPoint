...
Models within each app are stored in either `models.py` or within a submodule under the `models/` directory. When creating a model, be sure to subclass the [appropriate base model](models.md) from `netpoint.models`. This will typically be NetPointModel or OrganizationalModel. Remember to add the model class to the `__all__` listing for the module.
...
!!! info "Configuration Required"
    Set `DEVELOPER = True` in your NetPoint configuration to enable the creation of new migrations.
...
Each model should have a corresponding FilterSet class defined. This is used to filter UI and API queries. Subclass the appropriate class from `netpoint.filtersets` that matches the model's parent class.
...
If this model will be included in global search results, create a subclass of `netpoint.search.SearchIndex` for it and specify the fields to be indexed.
...
Add the relevant navigation menu items in `netpoint/netpoint/navigation/menu.py`.
...
Create a Graphene object type for the model in `graphql/types.py` by subclassing the appropriate class from `netpoint.graphql.types`.
