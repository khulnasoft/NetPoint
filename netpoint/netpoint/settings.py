...
from netpoint.config import PARAMS
from netpoint.constants import RQ_QUEUE_DEFAULT, RQ_QUEUE_HIGH, RQ_QUEUE_LOW
...
if sys.version_info < (3, 8):
    raise RuntimeError(
        f"NetPoint requires Python 3.8 or later. (Currently installed: Python {platform.python_version()})"
    )
...
config_path = os.getenv('NETPOINT_CONFIGURATION', 'netpoint.configuration')
try:
    configuration = importlib.import_module(config_path)
except ModuleNotFoundError as e:
    if getattr(e, 'name') == config_path:
        raise ImproperlyConfigured(
            f"Specified configuration module ({config_path}) not found. Please define netpoint/netpoint/configuration.py "
            f"per the documentation, or specify an alternate module in the NETPOINT_CONFIGURATION environment variable."
        )
    raise
...
REMOTE_AUTH_BACKEND = getattr(configuration, 'REMOTE_AUTH_BACKEND', 'netpoint.authentication.RemoteUserBackend')
...
SEARCH_BACKEND = getattr(configuration, 'SEARCH_BACKEND', 'netpoint.search.backends.CachedValueSearchBackend')
...
if RELEASE_CHECK_URL:
    validator = URLValidator(
        message=(
            ...
            "https://api.github.com/repos/khulnasoft/netpoint"
        )
    )
    try:
...
MIDDLEWARE = [
    ...
    'netpoint.middleware.RemoteUserMiddleware',
    'netpoint.middleware.CoreMiddleware',
    'netpoint.middleware.MaintenanceModeMiddleware',
...
TEMPLATES = [
    {
        ...
        'OPTIONS': {
            ...
            'context_processors': [
                ...
                'netpoint.context_processors.settings_and_registry',
            ],
        },
    },
]
...
AUTHENTICATION_BACKENDS = [
    *REMOTE_AUTH_BACKEND,
    'netpoint.authentication.ObjectPermissionBackend',
]
...
# WSGI
WSGI_APPLICATION = 'netpoint.wsgi.application'
...
CENSUS_URL = 'https://census.netpoint.dev/api/v1/'
CENSUS_PARAMS = {
    'version': VERSION,
...
SOCIAL_AUTH_PIPELINE = (
    ...
    'netpoint.authentication.user_default_groups_handler',
...
REST_FRAMEWORK = {
    ...
    'DEFAULT_AUTHENTICATION_CLASSES': (
        ...
        'netpoint.api.authentication.TokenAuthentication',
    ),
    'DEFAULT_FILTER_BACKENDS': (
        'django_filters.rest_framework.DjangoFilterBackend',
        'rest_framework.filters.OrderingFilter',
    ),
    'DEFAULT_METADATA_CLASS': 'netpoint.api.metadata.BulkOperationMetadata',
    'DEFAULT_PAGINATION_CLASS': 'netpoint.api.pagination.OptionalLimitOffsetPagination',
    'DEFAULT_PARSER_CLASSES': (
        'rest_framework.parsers.JSONParser',
        'rest_framework.parsers.MultiPartParser',
    ),
    'DEFAULT_PERMISSION_CLASSES': (
        'netpoint.api.authentication.TokenPermissions',
    ),
    'DEFAULT_RENDERER_CLASSES': (
        'rest_framework.renderers.JSONRenderer',
        'netpoint.api.renderers.FormlessBrowsableAPIRenderer',
    ),
    'DEFAULT_SCHEMA_CLASS': 'core.api.schema.NetPointAutoSchema',
