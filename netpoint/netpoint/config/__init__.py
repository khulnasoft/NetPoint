...
logger = logging.getLogger('netpoint.config')


def get_config():
    """
    Return the current NetPoint configuration, pulling it from cache if not already loaded in memory.
    """
...
class Config:
    """
    Fetch and store in memory the current NetPoint configuration. This class must be instantiated prior to access, and
