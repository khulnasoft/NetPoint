...
class ObjectChildrenView(ObjectView, ActionsMixin, TableMixin):
    """
    Display a table of child objects associated with the parent object. For example, NetPoint uses this to display
...
class ObjectEditView(GetReturnURLMixin, BaseObjectView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        Args:
            request: The current request
        """
        logger = logging.getLogger('netpoint.views.ObjectEditView')
        obj = self.get_object(**kwargs)
...
class ObjectDeleteView(GetReturnURLMixin, BaseObjectView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        Args:
            request: The current request
        """
        logger = logging.getLogger('netpoint.views.ObjectDeleteView')
        obj = self.get_object(**kwargs)
...
class ComponentCreateView(GetReturnURLMixin, BaseObjectView):
    ...
    def post(self, request):
        logger = logging.getLogger('netpoint.views.ComponentCreateView')
        form = self.initialize_form(request)
        instance = self.alter_object(self.queryset.model(), request)
