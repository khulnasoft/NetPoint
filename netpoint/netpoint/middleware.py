...
from netpoint.config import clear_config, get_config
from netpoint.views import handler_500
...
class RemoteUserMiddleware(RemoteUserMiddleware_):
    ...
    def process_request(self, request):
        logger = logging.getLogger(
            'netpoint.authentication.RemoteUserMiddleware')
...
class MaintenanceModeMiddleware:
    ...
    def process_exception(self, request, exception):
        ...
        if get_config().MAINTENANCE_MODE and isinstance(exception, InternalError):
            error_message = 'NetPoint is currently operating in maintenance mode and is unable to perform write ' \
