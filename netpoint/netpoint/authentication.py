...
class RemoteUserBackend(_RemoteUserBackend):
    ...
    def configure_groups(self, user, remote_groups):
        logger = logging.getLogger('netpoint.auth.RemoteUserBackend')
...
class LDAPBackend:
    ...
    def __new__(cls, *args, **kwargs):
        ...
        try:
            from netpoint import ldap_config
        except ModuleNotFoundError as e:
...
def user_default_groups_handler(backend, user, response, *args, **kwargs):
    ...
    configuration file.
    """
    logger = logging.getLogger('netpoint.auth.user_default_groups_handler')
