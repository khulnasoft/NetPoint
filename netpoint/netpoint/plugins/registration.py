...
def register_template_extensions(class_list):
    ...
    for template_extension in class_list:
        ...
        if not issubclass(template_extension, PluginTemplateExtension):
            raise TypeError(f"{template_extension} is not a subclass of netpoint.plugins.PluginTemplateExtension!")
...
def register_menu(menu):
    if not isinstance(menu, PluginMenu):
        raise TypeError(f"{menu} must be an instance of netpoint.plugins.PluginMenu")
...
def register_menu_items(section_name, class_list):
    ...
    for menu_link in class_list:
        if not isinstance(menu_link, PluginMenuItem):
            raise TypeError(f"{menu_link} must be an instance of netpoint.plugins.PluginMenuItem")
        for button in menu_link.buttons:
            if not isinstance(button, PluginMenuButton):
                raise TypeError(f"{button} must be an instance of netpoint.plugins.PluginMenuButton")
...
def register_graphql_schema(graphql_schema):
    """
    Register a GraphQL schema class for inclusion in NetPoint's GraphQL API.
    """
