...
from netpoint.search import register_search
from netpoint.utils import register_data_backend
...
class PluginConfig(AppConfig):
    """
    Subclass of Django's built-in AppConfig class, to be used for NetPoint plugins.
    """
    # Plugin metadata
    author = ''
    author_email = ''
    ...
    # Minimum/maximum compatible versions of NetPoint
    min_version = None
    max_version = None
    ...
    @classmethod
    def validate(cls, user_config, netpoint_version):

        # Enforce version constraints
        current_version = version.parse(netpoint_version)
        if cls.min_version is not None:
            min_version = version.parse(cls.min_version)
            if current_version < min_version:
                raise ImproperlyConfigured(
                    f"Plugin {cls.__module__} requires NetPoint minimum version {cls.min_version}."
                )
        if cls.max_version is not None:
            max_version = version.parse(cls.max_version)
            if current_version > max_version:
                raise ImproperlyConfigured(
                    f"Plugin {cls.__module__} requires NetPoint maximum version {cls.max_version}."
                )
