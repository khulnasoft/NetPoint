...
class NetPointModelForm(BootstrapMixin, CheckLastUpdatedMixin, CustomFieldsMixin, TagsMixin, forms.ModelForm):
    """
    Base form for creating & editing NetPoint models. Extends Django's ModelForm to add support for custom fields.
...
class NetPointModelImportForm(CSVModelForm, NetPointModelForm):
    """
    Base form for creating a NetPoint objects from CSV data. Used for bulk importing.
    """
...
class NetPointModelBulkEditForm(BootstrapMixin, CustomFieldsMixin, forms.Form):
    """
    Base form for modifying multiple NetPoint objects (of the same type) in bulk via the UI. Adds support for custom
...
class NetPointModelFilterSetForm(BootstrapMixin, CustomFieldsMixin, SavedFiltersMixin, forms.Form):
    """
    Base form for FilerSet forms. These are used to filter object lists in the NetPoint UI. Note that the
