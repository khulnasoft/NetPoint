...
class ClusterTypeForm(NetPointModelForm):
    slug = SlugField()
...
class ClusterGroupForm(NetPointModelForm):
    slug = SlugField()
...
class ClusterForm(TenancyForm, NetPointModelForm):
...
class VirtualMachineForm(TenancyForm, NetPointModelForm):
    site = DynamicModelChoiceField(
        label=_('Site'),
        queryset=Site.objects.all(),
        required=False
...
class VMComponentForm(NetPointModelForm):
