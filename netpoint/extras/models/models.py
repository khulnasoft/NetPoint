...
from netpoint.models import ChangeLoggedModel
from netpoint.models.features import (
...
class Webhook(CustomFieldsMixin, ExportTemplatesMixin, TagsMixin, ChangeLoggedModel):
    ...
    delete in NetPoint. The request will contain a representation of the object, which the remote application can act on.
...
class CustomLink(CloningMixin, ExportTemplatesMixin, ChangeLoggedModel):
    """
    A custom link to an external representation of a NetPoint object. The link text and URL fields accept Jinja2 template
...
class ExportTemplate(SyncedDataMixin, CloningMixin, ExportTemplatesMixin, ChangeLoggedModel):
    ...
    def render_to_response(self, queryset):
        ...
        if self.as_attachment:
            ...
            filename = f'netpoint_{basename}{extension}'
...
class JournalEntry(CustomFieldsMixin, CustomLinksMixin, TagsMixin, ExportTemplatesMixin, ChangeLoggedModel):
    ...
    preserve historical context around an object, and complements NetPoint's built-in change logging. For example, you
