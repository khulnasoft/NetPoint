...
from netpoint.forms.base import NetPointModelFilterSetForm
from netpoint.forms.mixins import SavedFiltersMixin
...
class WebhookFilterForm(NetPointModelFilterSetForm):
    model = Webhook
    fieldsets = (
...
class EventRuleFilterForm(NetPointModelFilterSetForm):
    model = EventRule
    tag = TagFilterField(model)
...
class JournalEntryFilterForm(NetPointModelFilterSetForm):
    model = JournalEntry
    fieldsets = (
        (None, ('q', 'filter_id', 'tag')),
