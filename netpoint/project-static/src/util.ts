...
/**
 * Fetch data from the NetPoint API (authenticated).
 ...
 * @param offset Y Offset. 0 by default, to take into account the NetPoint header.
 */
...
export function getNetboxData(key: string): string | null {
  ...
  for (const element of getElements('body > div#netpoint-data > *')) {
