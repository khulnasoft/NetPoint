...
export class APISelect {
  ...
  /**
   * Event that will initiate the API call to NetPoint to load option data. By default, the trigger
  ...
  constructor(base: HTMLSelectElement) {
    ...
    this.loadEvent = new Event(`netpoint.select.onload.${base.name}`);
    this.bottomEvent = new Event(`netpoint.select.atbottom.${base.name}`);
  ...
  private addEventListeners(): void {
    ...
    this.base.addEventListener(`netpoint.select.atbottom.${this.name}`, () =>
    ...
    this.base.addEventListener(`netpoint.select.disabled.${this.name}`, event =>
    ...
    for (const dep of dependencies) {
      ...
      this.base.addEventListener(`netpoint.select.onload.${dep}`, event => this.handleEvent(event));
    }
  }

  /**
   * Load this element's options from the NetPoint API.
   */
  ...
  /**
   * Query the NetPoint API for this element's options.
   */
   ...
   * desired action.
   *
   * @param event Dispatched event matching pattern `netpoint.select.disabled.<name>`
   */
  ...
  private setOptionStyles(): void {
    for (const option of this.options) {
      ...
      if (
        ...
        style.setAttribute('data-netpoint', id);
