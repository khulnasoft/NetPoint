 ...
 *
 * @param mode {"dark" | "light"} NetPoint Color Mode.
 * @param inferred {boolean} Value is inferred from browser/system preference.
 */
function setMode(mode, inferred) {
    document.documentElement.setAttribute("data-netpoint-color-mode", mode);
    localStorage.setItem("netpoint-color-mode", mode);
    localStorage.setItem("netpoint-color-mode-inferred", inferred);
}
/**
...
function initMode() {
    try {
        ...
        // Client NetPoint color-mode override.
        var clientMode = localStorage.getItem("netpoint-color-mode");
        // NetPoint server-rendered value.
        var serverMode = document.documentElement.getAttribute("data-netpoint-color-mode");
        // Color mode is inferred from browser/system preference and not deterministically set by
        // the client or server.
        var inferred = JSON.parse(localStorage.getItem("netpoint-color-mode-inferred"));
