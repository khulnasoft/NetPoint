...
class ProviderForm(NetPointModelForm):
    slug = SlugField()
...
class ProviderAccountForm(NetPointModelForm):
...
class ProviderNetworkForm(NetPointModelForm):
    provider = DynamicModelChoiceField(
        label=_('Provider'),
        queryset=Provider.objects.all()
    )
...
class CircuitTypeForm(NetPointModelForm):
    slug = SlugField()
...
class CircuitForm(TenancyForm, NetPointModelForm):
    provider = DynamicModelChoiceField(
        label=_('Provider'),
        queryset=Provider.objects.all(),
        selector=True
...
class CircuitTerminationForm(NetPointModelForm):
