...
class ContentTypePermissionRequiredMixin(AccessMixin):
    ...
    and fits within NetPoint's custom permission enforcement system.
...
def register_model_view(model, name='', path=None, kwargs=None):
    """
    This decorator can be used to "attach" a view to any model in NetPoint. This is typically used to inject
    additional tabs within a model's detail view. For example, to add a custom tab to NetPoint's dcim.Site model:
