...
BANNER_TEXT = """### NetPoint interactive shell ({node})
### Python {python} | Django {django} | NetPoint {netpoint}
### lsmodels() will show available models. Use help(<model>) for more info.""".format(
    node=platform.node(),
    python=platform.python_version(),
    django=get_version(),
    netpoint=settings.VERSION
)


class Command(BaseCommand):
    help = "Start the Django shell with all NetPoint models already imported"
    django_models = {}
