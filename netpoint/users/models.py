...
class NetPointUserManager(UserManager.from_queryset(RestrictedQuerySet)):
    pass


class NetPointGroupManager(GroupManager.from_queryset(RestrictedQuerySet)):
    pass


class NetPointUser(User):
    """
    Proxy contrib.auth.models.User for the UI
    """
    objects = NetPointUserManager()
    ...
    def get_absolute_url(self):
        return reverse('users:netpointuser', args=[self.pk])
...
class NetPointGroup(Group):
    """
    Proxy contrib.auth.models.User for the UI
    """
    objects = NetPointGroupManager()
    ...
    def get_absolute_url(self):
        return reverse('users:netpointgroup', args=[self.pk])
...
@receiver(post_save, sender=NetPointUser)
