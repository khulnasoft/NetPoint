...
from netpoint.api.viewsets.mixins import ObjectValidationMixin
from netpoint.config import get_config
from netpoint.constants import ADVISORY_LOCK_KEYS
...
class ASNRangeViewSet(NetPointModelViewSet):
...
class ASNViewSet(NetPointModelViewSet):
...
class VRFViewSet(NetPointModelViewSet):
...
class AggregateViewSet(NetPointModelViewSet):
    queryset = Aggregate.objects.prefetch_related('rir').prefetch_related('tags')
    serializer_class = serializers.AggregateSerializer
    filterset_class = filtersets.AggregateFilterSet


class RoleViewSet(NetPointModelViewSet):
...
class PrefixViewSet(NetPointModelViewSet):
...
class IPRangeViewSet(NetPointModelViewSet):
...
class IPAddressViewSet(NetPointModelViewSet):
...
class FHRPGroupViewSet(NetPointModelViewSet):
...
class FHRPGroupAssignmentViewSet(NetPointModelViewSet):
...
class VLANGroupViewSet(NetPointModelViewSet):
    queryset = VLANGroup.objects.annotate_utilization().prefetch_related('tags')
    serializer_class = serializers.VLANGroupSerializer
    filterset_class = filtersets.VLANGroupFilterSet


class VLANViewSet(NetPointModelViewSet):
...
class ServiceTemplateViewSet(NetPointModelViewSet):
...
class ServiceViewSet(NetPointModelViewSet):
