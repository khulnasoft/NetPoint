...
from netpoint.filtersets import ChangeLoggedModelFilterSet, OrganizationalModelFilterSet, NetPointModelFilterSet
...
class VRFFilterSet(NetPointModelFilterSet, TenancyFilterSet):
...
class RouteTargetFilterSet(NetPointModelFilterSet, TenancyFilterSet):
...
class AggregateFilterSet(NetPointModelFilterSet, TenancyFilterSet):
...
class PrefixFilterSet(NetPointModelFilterSet, TenancyFilterSet):
    family = django_filters.NumberFilter(
        field_name='prefix',
        lookup_expr='family'
    )
...
class IPRangeFilterSet(TenancyFilterSet, NetPointModelFilterSet):
    family = django_filters.NumberFilter(
...
class IPAddressFilterSet(NetPointModelFilterSet, TenancyFilterSet):
    family = django_filters.NumberFilter(
        field_name='address',
        lookup_expr='family'
    )
...
class FHRPGroupFilterSet(NetPointModelFilterSet):
...
class VLANFilterSet(NetPointModelFilterSet, TenancyFilterSet):
    region_id = TreeNodeMultipleChoiceFilter(
        queryset=Region.objects.all(),
        field_name='site__region',
        lookup_expr='in',
...
class ServiceTemplateFilterSet(NetPointModelFilterSet):
...
class ServiceFilterSet(NetPointModelFilterSet):
