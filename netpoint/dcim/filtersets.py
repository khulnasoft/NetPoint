...
from netpoint.filtersets import (
    BaseFilterSet, ChangeLoggedModelFilterSet, OrganizationalModelFilterSet, NetPointModelFilterSet,
)
...
class SiteFilterSet(NetPointModelFilterSet, TenancyFilterSet, ContactModelFilterSet):
...
class RackFilterSet(NetPointModelFilterSet, TenancyFilterSet, ContactModelFilterSet):
    region_id = TreeNodeMultipleChoiceFilter(
        queryset=Region.objects.all(),
        field_name='site__region',
        lookup_expr='in',
...
class RackReservationFilterSet(NetPointModelFilterSet, TenancyFilterSet):
...
class DeviceTypeFilterSet(NetPointModelFilterSet):
...
class ModuleTypeFilterSet(NetPointModelFilterSet):
    manufacturer_id = django_filters.ModelMultipleChoiceFilter(
        queryset=Manufacturer.objects.all(),
        label=_('Manufacturer (ID)'),
    )
...
class VirtualDeviceContextFilterSet(NetPointModelFilterSet, TenancyFilterSet, PrimaryIPFilterSet):
...
class ModuleFilterSet(NetPointModelFilterSet):
    manufacturer_id = django_filters.ModelMultipleChoiceFilter(
...
class ModuleBayFilterSet(DeviceComponentFilterSet, NetPointModelFilterSet):
...
class DeviceBayFilterSet(DeviceComponentFilterSet, NetPointModelFilterSet):
...
class InventoryItemFilterSet(DeviceComponentFilterSet, NetPointModelFilterSet):
    parent_id = django_filters.ModelMultipleChoiceFilter(
...
class VirtualChassisFilterSet(NetPointModelFilterSet):
...
class CableFilterSet(TenancyFilterSet, NetPointModelFilterSet):
...
class PowerPanelFilterSet(NetPointModelFilterSet, ContactModelFilterSet):
    region_id = TreeNodeMultipleChoiceFilter(
        queryset=Region.objects.all(),
        field_name='site__region',
        lookup_expr='in',
...
class PowerFeedFilterSet(NetPointModelFilterSet, CabledObjectFilterSet, PathEndpointFilterSet, TenancyFilterSet):
    region_id = TreeNodeMultipleChoiceFilter(
        queryset=Region.objects.all(),
