...
from netpoint.filtersets import NetPointModelFilterSet, OrganizationalModelFilterSet
...
class TunnelFilterSet(NetPointModelFilterSet, TenancyFilterSet):
...
class TunnelTerminationFilterSet(NetPointModelFilterSet):
...
class IKEProposalFilterSet(NetPointModelFilterSet):
...
class IKEPolicyFilterSet(NetPointModelFilterSet):
...
class IPSecProposalFilterSet(NetPointModelFilterSet):
    encryption_algorithm = django_filters.MultipleChoiceFilter(
        choices=EncryptionAlgorithmChoices
    )
    authentication_algorithm = django_filters.MultipleChoiceFilter(
...
class IPSecPolicyFilterSet(NetPointModelFilterSet):
...
class IPSecProfileFilterSet(NetPointModelFilterSet):
    mode = django_filters.MultipleChoiceFilter(
...
class L2VPNFilterSet(NetPointModelFilterSet, TenancyFilterSet):
...
class L2VPNTerminationFilterSet(NetPointModelFilterSet):
