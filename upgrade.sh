#!/bin/bash
# This script will prepare NetPoint to run after the code has been upgraded to
# its most recent release.

# This script will invoke Python with the value of the PYTHON environment
# variable (if set), or fall back to "python3". Note that NetPoint v3.2+ requires
# Python 3.8 or later.

cd "$(dirname "$0")"

NETPOINT_VERSION="$(grep ^VERSION netpoint/netpoint/settings.py | cut -d\' -f2)"
echo "You are installing (or upgrading to) NetPoint version ${NETPOINT_VERSION}"
...
eval $COMMAND || {
  echo "--------------------------------------------------------------------"
  echo "ERROR: Unsupported Python version: ${PYTHON_VERSION}. NetPoint requires"
...
COMMAND="python3 netpoint/manage.py migrate"
echo "Applying database migrations ($COMMAND)..."
eval $COMMAND || exit 1

# Trace any missing cable paths (not typically needed)
COMMAND="python3 netpoint/manage.py trace_paths --no-input"
...
COMMAND="python3 netpoint/manage.py collectstatic --no-input"
echo "Collecting static files ($COMMAND)..."
eval $COMMAND || exit 1

# Delete any stale content types
COMMAND="python3 netpoint/manage.py remove_stale_contenttypes --no-input"
echo "Removing stale content types ($COMMAND)..."
eval $COMMAND || exit 1

# Rebuild the search cache (lazily)
COMMAND="python3 netpoint/manage.py reindex --lazy"
echo "Rebuilding search cache ($COMMAND)..."
eval $COMMAND || exit 1

# Delete any expired user sessions
COMMAND="python3 netpoint/manage.py clearsessions"
...
if [ -v WARN_MISSING_VENV ]; then
  ...
  echo ""
  echo "netpoint.service ExecStart:"
  echo "  ${VIRTUALENV}/bin/gunicorn"
  echo ""
  echo "netpoint-rq.service ExecStart:"
...
echo "Upgrade complete! Don't forget to restart the NetPoint services:"
echo "  > sudo systemctl restart netpoint netpoint-rq"